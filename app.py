...
st.set_page_config(
    ...
    page_icon="🛡️",
    layout="wide",
    initial_sidebar_state="expanded",   # desktop: open by default; JS handles mobile collapse
)
...
# ─── HERO SECTION ────────────────────────────────────────────────────────────
st.markdown("""
<div style="text-align:center; padding:clamp(24px,5vw,50px) 0 10px 0;">
    <div style="display:inline-block; background:rgba(0,212,255,0.06);
                border:1px solid #0d3a6a; border-radius:20px;
                padding:8px 22px;
                font-family:'Share Tech Mono',monospace; color:#0099cc;
                font-size:clamp(0.65rem,1.5vw,0.8rem); letter-spacing:2px; margin-bottom:18px;">
        ⚡  ADVANCED CYBERSECURITY TOOLKIT  ⚡
    </div>
    <div style="font-family:'Orbitron',monospace;
                font-size:clamp(1.6rem,5vw,3.2rem);
                font-weight:900; color:#00d4ff;
                text-shadow:0 0 40px rgba(0,212,255,0.65);
                line-height:1.1; letter-spacing:clamp(1px,0.5vw,3px);
                word-break:break-word;">
        🛡️ CYBER WINGS
    </div>
    <div style="font-family:'Rajdhani',sans-serif; color:#5599cc;
                font-size:clamp(0.9rem,2.5vw,1.25rem);
                margin-top:10px; letter-spacing:1px;">
        Web Vulnerability Intelligence Platform
    </div>
    <div style="width:clamp(120px,30vw,250px); height:2px;
                background:linear-gradient(90deg,transparent,#00d4ff,transparent);
                margin:22px auto;"></div>
    <div style="font-family:'Rajdhani',sans-serif; color:#7799bb;
...
for col, (val, label, icon) in zip([col1, col2, col3, col4], stats):
    with col:
        ...
        <div style="background:linear-gradient(135deg,#070e1d,#0b1626);
                    border:1px solid #0d2a4a; border-radius:14px;
                    padding:clamp(14px,3vw,22px) 10px; text-align:center;
                    box-shadow:0 0 25px rgba(0,80,200,0.12);">
            <div style="font-size:clamp(1.3rem,3vw,1.8rem);">{icon}</div>
            <div style="font-family:'Orbitron',monospace;
            ...
            <div style="font-family:'Rajdhani',sans-serif; color:#5588aa;
                        font-size:clamp(0.75rem,1.5vw,0.9rem); margin-top:4px;">
                {label}
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    for col, mod in zip(cols, row):
        with col:
            ...
            <div style="background:linear-gradient(135deg,#070e1d,#0c1829);
                ...
                <div style="font-size:clamp(1.5rem,3vw,2rem); margin-bottom:8px;">{mod['icon']}</div>
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px; flex-wrap:wrap;">
                    ...
                    <span style="background:{mod['color']}18; color:{mod['color']};
                                 ...
                                 white-space:nowrap;">{mod['badge']}</span>
                </div>
                <div style="font-family:'Rajdhani',sans-serif; color:#5588aa;
                            font-size:clamp(0.82rem,1.6vw,0.95rem); line-height:1.55;">
                    {mod['desc']}
                </div>
                <div style="margin-top:14px; font-family:'Share Tech Mono',monospace;
                            color:{mod['color']}; font-size:0.75rem; opacity:0.7;">
...
<div style="background:rgba(255,100,0,0.05); border:1px solid #442200;
    ...
    <div style="font-family:'Rajdhani',sans-serif; color:#886644;
                font-size:clamp(0.82rem,1.6vw,0.9rem); line-height:1.5;">
        This tool is intended <strong style="color:#ffaa55;">for educational and authorized testing purposes only.</strong><br>
        Scanning websites without explicit permission may be illegal. Always obtain proper authorization before testing.
    </div>
</div>
